...
\subsubsection{What to expect from a knowledge representation system}
\label{sect|evaluation-literature}

Several authors from fields that are connected to the robotics have previously
listed desirable features of systems aiming at cognitive abilities as rich as
possible.
...
\begin{itemize}
	...
	\item non-monotonic reasoning \fxfatal{give here a clear example},
...
features (Section~\ref{sect|features}) that define the dimensions of a
...
We try in section~\ref{sect|summary} to summarize the current approaches and to
relate them to the previously identified set of features. This leads us to
...
\fxnote{Add nice illustrations from MORSE}

In order to illustrate what we consider to be the required features of a
knowledge representation system for robotics, we propose the following
scenario.

While devising a scenario inevitably constraints the breadth of abilities we
examine to the ones actually needed for this specific scenario, we insist on
the fact that our main purpose is to demonstrate on a real case what
representational features are desirable.

We entitle our scenario ``the Brownie scenario''.
...
hardware and softwares architectures, and especially, they are not expected to
have the same knowledge representation system. These robots cooperate with a
human in a kitchen environment.

The main task of the scenario is the joint realization of a brownie, that
starts with the human injunction ``Let's make a brownie for tonight!''.

The scenario is successful if the task is achieved (the brownie is made) and:
...
We voluntarily do not try to detail the subtasks of the scenario and to define
...
A ``first-order'' analysis of this task leads to a first list of categories of
required features:
...
The first category can be further refined: to prepare and bake a brownie, the
...
In conjunction with the representation of the action \emph{make} (and it's
possibly attached common-knowledge), this should lead the robot to build and
...
recipe may provided such a list. If we omit the ``may'', this is a typical
...
illustrate some of them further in this scenario.

As mentioned, a recipe often provides a procedure (or a \emph{plan}). The robot
...
Moreover, procedures are often \emph{underspecified}: we can expect the recipe
...
self-evident that the door must first be opened. Such underspecification should
be detectable, representable, and ideally completable by the knowledge
representation system.

Besides representation of space and time, we may have

% Representation feature that enable cooperation
Then, we want our three agents to cooperate. This, in turn, leads to a very
different set of cognitive abilities.

Cooperation in our scenario can intervene at many levels: for instance, an
agent may want to inform another one about the number of eggs that are
necessary for the brownie. It makes sense to help in this way only if the first
agent knows that the recipient agent both needs the information and does not
know it. This in turn requires the robot to be able to model the knowledge of
the other agents: to think \emph{from the perspective} of another agent (this
idea is related to the so-called Theory of Mind~\cite{...}).
...
talking about? If we want to act on the world, this \emph{grounding} operation
is essential, and may be tightly bound to the underlying knowledge
...
the human during the cooking session, the robots need to store at least how far
...
may want to be sure these systems successfully interoperate, \ie that the
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{A typology of features of knowledge representation systems}
\label{sect|features}

The previous section detailed an imaginary cooking scenario. We identified, in
no particular order, numerous desirable features for a knowledge representation
system for robotics.
